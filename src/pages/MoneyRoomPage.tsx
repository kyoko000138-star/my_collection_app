...
// [긴급 수정] 키 변경으로 데이터 충돌 회피 (초기화)
const STORAGE_KEY = 'money-room-save-v13-emergency';
...
// [긴급 수정] 안전한 병합 로직 (undefined 방어)
const mergeUserState = (base: UserState, saved: any): UserState => {
  if (!saved) return base;
  return {
    ...base,
    ...saved,
    // 배열 데이터 강제 초기화 (length 오류 방지)
    ...
    unresolvedShadows: Array.isArray(saved.unresolvedShadows) ? saved.unresolvedShadows : [],
    unlockedLocations: Array.isArray(saved.unlockedLocations) ? saved.unlockedLocations : ['VILLAGE_BASE'],
    // 객체 데이터 병합
    lunaCycle: { 
      ...base.lunaCycle, 
      ...(saved.lunaCycle || {}), 
      history: Array.isArray(saved.lunaCycle?.history) ? saved.lunaCycle.history : [] 
    },
    assets: { ...base.assets, ...(saved.assets || {}) },
    garden: { 
      ...base.garden, 
      ...(saved.garden || {}), 
      decorations: Array.isArray(saved.garden?.decorations) ? saved.garden.decorations : [] 
    },
    stats: { ...base.stats, ...(saved.stats || {}) },
    counters: { ...base.counters, ...(saved.counters || {}) },
    currentLocation: saved.currentLocation || 'VILLAGE_BASE'
  };
};

const MoneyRoomPage: React.FC = () => {
  // [안전 로드] try-catch로 데이터 손상 시 초기화
  ...
  const handleOnboarding = (data: any) => {
    setGameState(prev => ({
      ...
      maxBudget: Number(data.budget.total) || 0,
      currentBudget: Number(data.budget.current) || 0,
